...
- **Vault integrations**: Password managers, secret stores, and enterprise SSO helpers fit `credential.read`. A trusted browser host that must keep plaintext out of agent-browser should instead implement `credential.inject` and return only fill and submit state.
...
For `credential.inject`, use request type `credential.inject` and return an `injection` object such as `{ "state": "filled", "usernameFilled": true, "submitted": true }`. Never include username, password, OTP, passkey material, or cookie values in this response.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>credential.inject</code></td><td><code>credential.inject</code></td><td><code>auth login --credential-provider &lt;name&gt;</code></td><td><code>injection</code></td></tr>
...
Plugins may declare custom capabilities such as `captcha.solve`. `plugin run` can invoke `command.run` and custom capabilities, but it cannot invoke core capabilities or protocol request types directly. Use the dedicated command path for `credential.read`, `credential.inject`, `browser.provider`, and `launch.mutate`.

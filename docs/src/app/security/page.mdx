...
- **Plugin secret access**: Legacy `credential.read` providers return a credential to agent-browser for one login. Trusted host applications should use `credential.inject`, which fills and submits locally and returns only non-secret state. Core agent-browser keeps policy checks and redaction-sensitive output handling.
...
Auth vault operations keep secrets out of normal command output and LLM context. Domain allowlist restrictions still apply to `auth login` navigations. Plugin-backed logins expose `plugin:<name>:credential.read` or the state-only `plugin:<name>:credential.inject` action for policy and confirmation gates.
